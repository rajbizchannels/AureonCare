...
--
-- Two earlier attempts at this script also failed, in the SQL editor only:
--   * nesting three levels of dollar quoting -> 42601 unterminated dollar-quoted string
--   * carrying the file bodies as one huge single-quoted literal -> 42601 syntax error at
--     "CREATE", because the editor breaks a literal of that size apart
-- So the migrations are inlined here as INDIVIDUAL statements in dollar-quoted blocks,
-- which is the shape the editor already handles. No giant literals, no nesting.
--
-- Covers every tenant schema plus template. Safe to re-run: the per-schema ledger is
-- honoured, so nothing is applied twice and run-tenant-migrations.js will not repeat it.
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE format('SELECT EXISTS (SELECT 1 FROM %I.schema_migrations WHERE version = 1)', sch)
      INTO done;
    IF NOT done THEN
      EXECUTE $s01$CREATE TABLE IF NOT EXISTS form_categories (
      ...
      EXECUTE $s02$CREATE TABLE IF NOT EXISTS form_templates (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      ...
      EXECUTE $s03$CREATE TABLE IF NOT EXISTS form_template_versions (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      ...
      EXECUTE $s04$CREATE TABLE IF NOT EXISTS form_submissions (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      ...
      EXECUTE $s05$CREATE TABLE IF NOT EXISTS form_signatures (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      ...
      EXECUTE $s06$CREATE TABLE IF NOT EXISTS form_audit_logs (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      ...
      EXECUTE $s07$CREATE TABLE IF NOT EXISTS intake_flow_templates (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      ...
      EXECUTE $s08$CREATE TABLE IF NOT EXISTS laboratories (
          id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      ...
      EXECUTE $s09$CREATE TABLE IF NOT EXISTS campaigns (
      ...
      EXECUTE $s10$CREATE TABLE IF NOT EXISTS clinic_working_hours (
        id SERIAL PRIMARY KEY,
      ...
      EXECUTE $s11$CREATE TABLE IF NOT EXISTS clinic_appointment_settings (
        id SERIAL PRIMARY KEY,
      ...
      EXECUTE $s12$CREATE TABLE IF NOT EXISTS telehealth_provider_settings (
          id SERIAL PRIMARY KEY,
      ...
      EXECUTE $s13$CREATE TABLE IF NOT EXISTS backup_provider_settings (
            id SERIAL PRIMARY KEY,
            provider_type VARCHAR(50) UNIQUE NOT NULL,
            is_enabled BOOLEAN DEFAULT false,
      ...
      EXECUTE $s14$CREATE TABLE IF NOT EXISTS vendor_integration_settings (
          id SERIAL PRIMARY KEY,
      ...
      EXECUTE $s15$CREATE TABLE IF NOT EXISTS offering_form_links (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    UNIQUE(offering_id, form_template_id)
      ...
      EXECUTE $s16$ALTER TABLE lab_orders ALTER COLUMN result_recipients TYPE JSONB USING result_recipients::jsonb$s16$;
      EXECUTE $s17$ALTER TABLE prescriptions
        ADD COLUMN IF NOT EXISTS diagnosis_id UUID REFERENCES diagnosis(id) ON DELETE SET NULL$s17$;
      EXECUTE format('INSERT INTO %I.schema_migrations (version, name) VALUES (1, %L)',
                     sch, '001_adopt_runtime_created_tables.sql');
    ...
    IF NOT done THEN
      ...
      EXECUTE format(
        'CREATE TRIGGER audit_logs_no_mutate BEFORE UPDATE OR DELETE ON %I.audit_logs
           FOR EACH ROW EXECUTE FUNCTION public.audit_logs_append_only()', sch);
      EXECUTE format('INSERT INTO %I.schema_migrations (version, name) VALUES (2, %L)',
                     sch, '002_audit_log_append_only.sql');
      n_applied := n_applied + 1;
    END IF;
    ...
    IF NOT done THEN
      ...
      EXECUTE format('INSERT INTO %I.schema_migrations (version, name) VALUES (3, %L)',
                     sch, '003_audit_logs_keep_history.sql');
      n_applied := n_applied + 1;
    END IF;
  END LOOP;
  ...
  LOOP
    ...
    EXECUTE format(
      'SELECT string_agg(version::text, '','' ORDER BY version) FROM %I.schema_migrations
        WHERE version BETWEEN 1 AND 3', sch) INTO got;
